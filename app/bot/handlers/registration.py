...
async def start_command(
    ...
    status = await registration_service.check_registration(
        message.from_user.id
    )

    if status.user and status.user.status == UserStatus.BLOCKED.value:
        await message.answer(
            "⛔️ دسترسی شما به چالش مسدود شده است.",
    ...
    if status.registered:
        await message.answer(
            ...
            "هر روز تمرین برای شما ارسال می‌شود.",
            reply_markup=ReplyKeyboardRemove(),
        )
        return
...
async def process_invalid_name(
    message: Message
) -> None:

    await message.answer(
        "لطفاً نام خود را به صورت متن ارسال کنید."
    )
...
async def process_phone(
    ...
    await message.answer(
        ...
        "تمرین‌های روزانه برای شما ارسال خواهد شد.",
        reply_markup=ReplyKeyboardRemove(),
    )
...
async def process_invalid_phone(
    ...
    await message.answer(
        "برای تکمیل ثبت‌نام شماره تماس را با دکمه تلگرام ارسال کنید.",
        reply_markup=contact_keyboard(),
    )